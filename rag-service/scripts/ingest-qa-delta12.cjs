...
const QA_DELTA_PATH = path.resolve(ROOT, 'originaldata', 'processed', 'qa_seed_delta12.json')
...
function printHelp() {
  console.log(`Usage:
  node scripts/ingest-qa-delta12.cjs [options]
...
async function main() {
  ...
  if (!qaPairs.length) {
    console.log('[ingest-qa-delta12] no qa pairs found')
    return
  }
  ...
  if (!response.ok) {
    throw new Error(`Failed to ingest QA delta12: ${response.status} ${JSON.stringify(response.data)}`)
  }
...
main().catch(error => {
  console.error('[ingest-qa-delta12] failed', error)
  process.exit(1)
})
