...
const QA_DELTA_PATH = path.resolve(ROOT, 'originaldata', 'processed', 'qa_seed_delta6.json')
...
function printHelp() {
  console.log(`Usage:
  node scripts/ingest-qa-delta6.cjs [options]
...
async function main() {
  ...
  if (!qaPairs.length) {
    console.log('[ingest-qa-delta6] no qa pairs found')
    return
  }
  ...
  if (!response.ok) {
    throw new Error(`Failed to ingest QA delta6: ${response.status} ${JSON.stringify(response.data)}`)
  }
...
main().catch(error => {
  console.error('[ingest-qa-delta6] failed', error)
  process.exit(1)
})
